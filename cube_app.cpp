...
CubeApp* CubeApp::appInstance = nullptr;
...
CubeApp::CubeApp() {
    ...
    window = glfwCreateWindow(800, 600, "Cube and Sphere Renderer", nullptr, nullptr);
    if (!window) {
    ...
    // Генерируем куб
    generateCubeMesh(cubeVertices, cubeIndices, 1.0f);
...
void CubeApp::createInstance() {
    VkApplicationInfo appInfo{};
    appInfo.sType = VK_STRUCTURE_TYPE_APPLICATION_INFO;
    appInfo.pApplicationName = "Cube Renderer";
...
void CubeApp::createSurface() {
    if (glfwCreateWindowSurface(instance, window, nullptr, &surface) != VK_SUCCESS) {
        throw std::runtime_error("failed to create window surface!");
    }
}

void CubeApp::pickPhysicalDevice() {
...
void CubeApp::createLogicalDevice() {
...
void CubeApp::createDepthResources() {
...
void CubeApp::createDescriptorSetLayout() {
...
void CubeApp::createGraphicsPipeline() {
...
void CubeApp::createFramebuffers() {
...
void CubeApp::createCommandPool() {
...
void CubeApp::createVertexBuffer() {
    ...
    createBuffer(device, physicalDevice, cubeBufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, cubeStagingBuffer, cubeStagingBufferMemory);
    
    void* cubeData;
    vkMapMemory(device, cubeStagingBufferMemory, 0, cubeBufferSize, 0, &cubeData);
    memcpy(cubeData, cubeVertices.data(), (size_t) cubeBufferSize);
    vkUnmapMemory(device, cubeStagingBufferMemory);
    
    createBuffer(device, physicalDevice, cubeBufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, cubeVertexBuffer, cubeVertexBufferMemory);
    
    copyBuffer(device, commandPool, graphicsQueue, cubeStagingBuffer, cubeVertexBuffer, cubeBufferSize);
    
    vkDestroyBuffer(device, cubeStagingBuffer, nullptr);
    vkFreeMemory(device, cubeStagingBufferMemory, nullptr);
    
    // Создаем буфер для сферы
    VkDeviceSize sphereBufferSize = sizeof(sphereVertices[0]) * sphereVertices.size();
    
    VkBuffer sphereStagingBuffer;
    VkDeviceMemory sphereStagingBufferMemory;
    createBuffer(device, physicalDevice, sphereBufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, sphereStagingBuffer, sphereStagingBufferMemory);
    
    void* sphereData;
    vkMapMemory(device, sphereStagingBufferMemory, 0, sphereBufferSize, 0, &sphereData);
    memcpy(sphereData, sphereVertices.data(), (size_t) sphereBufferSize);
    vkUnmapMemory(device, sphereStagingBufferMemory);
    
    createBuffer(device, physicalDevice, sphereBufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, sphereVertexBuffer, sphereVertexBufferMemory);
    
    copyBuffer(device, commandPool, graphicsQueue, sphereStagingBuffer, sphereVertexBuffer, sphereBufferSize);
...
void CubeApp::createIndexBuffer() {
    // Создаем буфер индексов для куба
    ...
    void* cubeData;
    vkMapMemory(device, cubeStagingBufferMemory, 0, cubeBufferSize, 0, &cubeData);
    memcpy(cubeData, cubeIndices.data(), (size_t) cubeBufferSize);
    vkUnmapMemory(device, cubeStagingBufferMemory);
    
    createBuffer(device, physicalDevice, cubeBufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, cubeIndexBuffer, cubeIndexBufferMemory);
    
    copyBuffer(device, commandPool, graphicsQueue, cubeStagingBuffer, cubeIndexBuffer, cubeBufferSize);
    
    vkDestroyBuffer(device, cubeStagingBuffer, nullptr);
    vkFreeMemory(device, cubeStagingBufferMemory, nullptr);
    
    // Создаем буфер индексов для сферы
    VkDeviceSize sphereBufferSize = sizeof(sphereIndices[0]) * sphereIndices.size();
    ...
    void* sphereData;
    vkMapMemory(device, sphereStagingBufferMemory, 0, sphereBufferSize, 0, &sphereData);
    memcpy(sphereData, sphereIndices.data(), (size_t) sphereBufferSize);
    vkUnmapMemory(device, sphereStagingBufferMemory);
    
    createBuffer(device, physicalDevice, sphereBufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, sphereIndexBuffer, sphereIndexBufferMemory);
    
    copyBuffer(device, commandPool, graphicsQueue, sphereStagingBuffer, sphereIndexBuffer, sphereBufferSize);
    
    vkDestroyBuffer(device, sphereStagingBuffer, nullptr);
    vkFreeMemory(device, sphereStagingBufferMemory, nullptr);
}

void CubeApp::createUniformBuffers() {
...
void CubeApp::createDescriptorPool() {
...
void CubeApp::createDescriptorSets() {
...
void CubeApp::createSyncObjects() {
...
void CubeApp::drawFrame() {
    ...
    vkCmdBindIndexBuffer(commandBuffers[currentFrame], cubeIndexBuffer, 0, VK_INDEX_TYPE_UINT32);
...
bool CubeApp::isDeviceSuitable(VkPhysicalDevice device) {
...
bool CubeApp::checkDeviceExtensionSupport(VkPhysicalDevice device) {
...
std::vector<const char*> CubeApp::getRequiredExtensions() {
...
VkSurfaceFormatKHR CubeApp::chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& availableFormats) {
...
VkPresentModeKHR CubeApp::chooseSwapPresentMode(const std::vector<VkPresentModeKHR>& availablePresentModes) {
...
VkExtent2D CubeApp::chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities) {
...
VkShaderModule CubeApp::createShaderModule(const std::vector<char>& code) {
...
void CubeApp::mouseCallback(GLFWwindow* window, double xpos, double ypos) {
    if (appInstance) {
...
VkFormat CubeApp::findDepthFormat() {
...
VkFormat CubeApp::findSupportedFormat(const std::vector<VkFormat>& candidates, VkImageTiling tiling, VkFormatFeatureFlags features) {
...
void CubeApp::createImage(VkDevice device, VkPhysicalDevice physicalDevice, uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory) {
...
VkImageView CubeApp::createImageView(VkDevice device, VkImage image, VkFormat format, VkImageAspectFlags aspectFlags) {
